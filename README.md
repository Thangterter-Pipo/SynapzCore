...
- 🧠 **Shared Memory**: Supabase cloud — bộ nhớ dài hạn tập trung (tự động lưu, đo mức độ quan trọng).
...
- 📊 **Admin Dashboard**: Giao diện điều khiển trung tâm (`scripts/dashboard.html`) — xem ký ức, quản lý tasks, theo dõi tiến trình và tương tác với IDE.
- 🤝 **Multi-Agent Coordination**: `scripts/coordination.py` — khóa file (file locking),## Kiến Trúc
...
graph TD
    ...
    subgraph RustWorkspace["📦 SynapzCore (E:\\AGT_Brain)"]
        Builder["🤖 Antigravity (THE BUILDER & ORCHESTRATOR)<br>Main Agent"]
...
*   team_memory: Đồng bộ hóa ký ức có độ quan trọng cao dùng chung của hệ thống.
