...
pub type ToolFn = Box<dyn Fn(Value) -> Pin<Box<dyn Future<Output = Result<Value>> + Send>> + Send + Sync>;
...
impl Registry {
        ...
        Fut: Future<Output = Result<Value>> + Send + 'static,
    {
        ...
        self.tools.insert(name_str.clone(), Tool {
            name: name_str,
    ...
    pub async fn execute(&self, name: &str, params: Value) -> Result<Value> {
        let tool = self.tools.get(name)
            .ok_or_else(|| anyhow::anyhow!("Tool '{}' not found. Available: {:?}", name, self.list_names()))?;
    ...
    pub fn list(&self) -> Vec<(&str, &str)> {
        self.tools.values().map(|t| (t.name.as_str(), t.description.as_str())).collect()
    }
...
pub fn build_default_registry() -> Registry {
    ...
    r.register("append_file", "Thêm nội dung vào cuối file", file::append_file);
    r.register("list_dir", "Liệt kê nội dung thư mục", file::list_dir);
    r.register("search_files", "Tìm file theo glob pattern", file::search_files);
    r.register("file_exists", "Kiểm tra file có tồn tại", file::file_exists);

    // Shell tools (1)
    r.register("run_command", "Chạy lệnh shell (có blocklist)", shell::run_command);
    ...
    r.register("remember", "Tìm ký ức (semantic search, filter by agent)", memory::remember);
    r.register("save_memory", "Lưu ký ức mới (with agent/category/importance)", memory::save_memory);
    r.register("recall_boss", "Lấy profile Bố từ memory", memory::recall_boss);
